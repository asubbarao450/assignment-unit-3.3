...
//create 3 arrays for positive, negative and zero values
let positives = [];
let negatives = [];
let zeroes = [];
...
for (let i = 0; i <= supplyChanges.length - 1; i++) {
  ...
  //conditionals for evaluating if array item to sort is positive, negative or zero. 
...
//for each loop
for (let q of supplyChanges) {

  //conditionals below to evaluate array item is pos, neg or zero
  if (q == 0) {
    stretchZeroes.push(q);
  }
  else if (q > 0) {
    stretchPositives.push(q);
  }
  else if (q < 0) {
    stretchNegatives.push(q);
  }
...
let parts = 572;
let boxesFilled = 0;
let numofPartsinBox = 0

//parts >0 (and not parts >=0) because we want to run the loop when only one part remains. 
//If 0 parts remain we do not want to run the loop. If parts >1 is used then the remaing parts tally at the end will be incorrect.

while (parts > 0) {

  //removes item from parts pile
  parts -= 1;

  //increment represents addition to a box
  numofPartsinBox++;

  //if the numOfParts reaches 7 then the box is full. 
  //we increment a full box and then set the numofParts count to 0
  if (numofPartsinBox == 7) {
    boxesFilled++;
    numofPartsinBox = 0;
  }
...
console.log(parts);
//parts is exhausted and the remaining count of parts is represented by numofPartsinBox
parts=numofPartsinBox;

console.log(boxesFilled);
console.log(numofPartsinBox);
